...
static volatile bool init_scan = false;
...
void app_main(void)
{
    ...
    // Turn LED OFF when time obtained
    ...
    snprintf(filename, sizeof(filename), CONFIG_SD_MOUNT_POINT"/"CONFIG_TIMES_FILENAME);
    ...
    while (true)
    {
        if (init_scan)
        {
            ...
            timeinfo2 = localtime(&current_time);
            strftime(strftime_buf, sizeof(strftime_buf), "%c", timeinfo2);
            fprintf(file, "%s, ", strftime_buf);

            for (uint8_t i = 0; i < SCANS_PER_CYCLE; i++)
            {
                wifi_scan();
                vTaskDelay(CONFIG_SCAN_DELAY_MS / portTICK_PERIOD_MS);
            }
            ...
            // Turn LED OFF when scan finishes
            ESP_ERROR_CHECK(gpio_set_level(CONFIG_GPIO_LED_PIN, CONFIG_GPIO_LED_OFF));
